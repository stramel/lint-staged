...
class GitWorkflow {
  ...
  async getPartiallyStagedFiles() {
    ...
    const partiallyStaged = status
      ...
      .reduce((fixed, file) => {
        // if file has been renamed, split it to two
        const RENAME = ' -> '
        return fixed.concat(file.includes(RENAME) ? file.split(RENAME) : file)
      }, [])
   ...
   * This fails when a file has been renamed, since of the entries points to a file not on disk
   * It should be safe to ignore errors here
   */
  async hideUnstagedChanges() {
    try {
      await Promise.all(
        this.partiallyStagedFiles.map(file => this.execGit(['checkout', '--force', '--', file]))
      )
  ...
  async restoreUnstagedChanges(ctx) {
    ...
    const unstagedPatch = this.getHiddenFilepath(PATCH_UNSTAGED)
    const include = this.partiallyStagedFiles.map(file => `--include=${file}`)
    try {
      await this.execGit(['apply', ...GIT_APPLY_ARGS, ...include, unstagedPatch])
    } catch (applyError) {
  ...
  async restoreOriginalState(ctx) {
    try {
      ...
      // Clean out patches
      await Promise.all([unlink(PATCH_UNSTAGED)])
    } catch (error) {
